...
PROMPT_MAP  = {"Theory": THEORY_PROMPT, "Characteristics": CHARACTERISTICS_PROMPT, "Method": METHOD_PROMPT}
MODEL_NAMES = list(EXTRACTION_MODELS.keys())   # ["Mistral-7B", "Zephyr-7B", "Qwen2.5-7B"]
...
def run_council(pdfs, inventory_file, journal, prompt_type, progress=gr.Progress()):
    ...
    s4 = council_graph.invoke({"sheet1": s1, "sheet2": s2, "sheet3": s3, "consolidated": []})["consolidated"]
    ...
    return json.dumps(s1[:5], indent=2), json.dumps(s2[:5], indent=2), json.dumps(s3[:5], indent=2), json.dumps(s4[:5], indent=2), xlsx
...
with gr.Blocks(title="Agentic TCCM Extractor — LLM Council") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            pdfs          = gr.File(label="Paper PDFs", file_count="multiple", file_types=[".pdf"])
            inventory_f   = gr.File(label="IS_Theories (.xlsx / .txt)", file_types=[".xlsx", ".txt", ".csv"])
            journal       = gr.Textbox(label="Journal Name", value="MIS Quarterly")
            prompt_type   = gr.Radio(["Theory", "Characteristics", "Method"],
                                     label="Extraction Prompt", value="Theory")
